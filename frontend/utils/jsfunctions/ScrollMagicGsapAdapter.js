...
ScrollMagic.Scene.extend(function () {
     ...
     * @private
     */
    const updateTweenProgress = function () {
      if (_tween) {
        const
     ...
     * const tween = TweenMax.to("obj"), 1, {x: 100};
     * scene.setTween(tween);
     *
     * // add multiple tweens, wrapped in a timeline.
     * const timeline = new TimelineMax();
     * const tween1 = TweenMax.from("obj1", 1, {x: 100});
     * const tween2 = TweenMax.to("obj2", 1, {y: 100});
     * timeline
     *    .add(tween1)
     *    .add(tween2);
    ...
    Scene.setTween = function (TweenObject, duration, params) {
      ...
      if (parseFloat(TweenLite.version) >= 1.14) { // onOverwrite only present since GSAP v1.14.0
        ...
        for (let i = 0, thisTween, oldCallback; i < list.length; i++) { /*jshint loopfunc: true */
          thisTween = list[i];
